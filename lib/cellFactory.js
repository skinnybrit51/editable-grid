...
module.exports = {
    ...
    createOpeningCellTag: function (options) {
        var column = options.column,
            treeColumn = options.treeColumn,
            level = options.level,
            getCellClassesFn = options.getCellClassesFn,
            obj = options.obj;

        var classes = [];

        if (_.isFunction(getCellClassesFn)) {
            _.each(getCellClassesFn(column.id, obj), function (clzz) {
                classes.push(clzz);
            });
        }
    ...
    createSelect: function (column, value) {
        // opening tag
        var cell = this.createOpeningCellTag({ column: column });
    ...
    createTableData: function (options) {
        ...
        var markup = this.createOpeningCellTag({
            column: column,
            treeColumn: treeColumn,
            level: level,
            obj: options.obj,
            getCellClassesFn: getCellClassesFn
        });
