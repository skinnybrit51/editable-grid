...
var cellFactory = {
    createTableData: function (options) {
        return '<td data-cell-id="' + options.id + '" style="width:' + options.width + '">'+
            options.value + '</td>';
    },

    createTableHeader: function (options) {
        return '<th data-cell-id="' + options.id + '" style="width:' + options.width + '">' +
            options.value + '</th>';
    }
};
...
module.exports = {
    ...
    createTableRow: function (options) {
        var row = '<tr data-row-id="' + options.data.id + '">';

        _.each(options.columns, function (column) {
            var value = options.data[column.id];
